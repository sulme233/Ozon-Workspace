...
refund_prod = odf[refund_kw].groupby(['商品id','商品']).agg(退款订单数=('订单号','count')).reset_index()
...
for c in ['净成交花费(元)','净交易额(元)','净成交笔数']:
    pdf[c] = pd.to_numeric(pdf[c], errors='coerce').fillna(0)
prod = pdf.groupby(['商品ID','商品名称']).agg(广告花费=('净成交花费(元)','sum'), 广告成交额=('净交易额(元)','sum'), 广告订单数=('净成交笔数','sum')).reset_index()
...
prod = prod.merge(refund_prod, left_on='商品ID', right_on='商品id', how='left', suffixes=('','_r')).drop(columns=['商品id'], errors='ignore')
prod = prod.fillna(0)
...
print(prod[['商品ID','商品名称','广告花费','广告成交额','广告订单数','ROAS','店铺销售额','店铺订单数','退款订单数','问题判断','优化建议','操作建议']].head(20).to_json(orient='records', force_ascii=False))
